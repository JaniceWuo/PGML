...
class BaseModel():    
    ...
    def build_model(self):
        ...
        with tf.variable_scope(self.initial_scope):
            ...
            self.trunc_norm_init = tf.truncated_normal_initializer(stddev=self.hps.trunc_norm_init_std)  #trunc_norm_init_std default=1e-3
    ...
    def add_encoder(self):
        ...
        # print("self.emb_enc_inp:",self.emb_enc_inp)  #shape=(16, ?, 300)
        ...
        # self.embedding_matrix = make_custom_embedding_matrix(self.vocab, self.hps)
        word_all_emb = tf.expand_dims(self.embedding_matrix,1)
        vocab_add_len = tf.ones([self.hps.batch_size], dtype=tf.int32)
        def add_word(index):
            word_add = word_all_emb[index,:,:]
            word_add = tf.expand_dims(word_add,0)
            multi = tf.tile(word_add, multiples=[self.hps.batch_size, 1, 1])
            final_sequence = tf.concat([self.emb_enc_inp,multi],axis=1)
            final_len = self.enc_lens + vocab_add_len
            with tf.variable_scope('word', reuse=tf.AUTO_REUSE):
                cell_lstm = tf.nn.rnn_cell.BasicLSTMCell(self.hps.hidden_dim)
                cell_lstm = tf.contrib.rnn.DropoutWrapper(cell=cell_lstm, input_keep_prob=self.keep_rate)
                _init_state = cell_lstm.zero_state(self.hps.batch_size, dtype=tf.float32)
                word_outputs, _ = tf.nn.dynamic_rnn(cell_lstm, final_sequence,initial_state=_init_state, dtype=tf.float32, sequence_length=final_len)
                # final_step = word_states[1]
                final_step = word_outputs[:,-1,:] 
                # logits = tf.layers.dense(inputs=final_step, units=self.hps.batch_size) 
                prob = tf.reduce_max(tf.nn.softmax(final_step),reduction_indices=[1])  

                one_word_plogp = tf.multiply(prob,tf.log(prob)) 
                # print("one_word_plogp的shape",one_word_plogp)  #shape=(16,)
            # return one_word_sum/self.hps.batch_size
            return one_word_plogp
        word_output = []
        wordnum = 1000
        word_index_list_random = random.sample(range(0,49991), wordnum)
        word_output = add_word(word_index_list_random[0])
        for i in range(1,wordnum):
            word_output = tf.add(word_output,add_word(word_index_list_random[i]))
        # print("word_output",word_output)  #shape=(16,)
    ...
    def add_train_op(self):
        ...
        self.gradients = tf.gradients(loss_to_minimize, tvars, aggregation_method=tf.AggregationMethod.EXPERIMENTAL_TREE)
        # print("gradients:",gradients) 
        grads, global_norm = tf.clip_by_global_norm(self.gradients, self.hps.max_grad_norm)
    ...
    def make_feeddict(self, batch, encoder_only=False):
        ...
        # print("self.enc_batch:",self.enc_batch)
        # print(feed_dict[self.enc_batch])
