...
def attention_decoder(emb_dec_inp, dec_init_state, enc_outputs, enc_pad_masks, cell, initial_state_attention):
    with variable_scope.variable_scope('attention_decoder', reuse=tf.AUTO_REUSE) as scope:
        batch_size = enc_outputs.get_shape()[0].value   #enc_outputs的shape为[batch_size, max_enc_len, 2 * hidden_dim]

        # length of encoded input to attend                
        attn_lens = enc_outputs.get_shape()[2].value          #2代表获取第2维的值  [0,2,3]则为3  在这里应该为2*hidden_dim

        # [batch_size, attn_lens, 1, attn_size)     ？？？ 我觉得不对诶  应该是[batch_size, max_enc_len, 1, attn_lens]
        ...
        def attention(decoder_state):
            with tf.variable_scope('attention', reuse=tf.AUTO_REUSE) as attn_scope:
                ...
                def masked_attention(e):
                    ...
                    masked_sums = tf.reduce_sum(attn_dist, axis=1)  # shape (batch_size)   reduce_sum理解为压缩求和，用于降维 axis=1为按行求和
        ...
        if initial_state_attention:  # true in decode mode     如果是train时 则不执行这一步
        ...
        for i, inp in enumerate(emb_dec_inp):
            if i > 0:
                variable_scope.get_variable_scope().reuse_variables()  #scope.reuse_variables()触发重用变量 让变量共享

            # Merge input and previous attentions into one vector x of the same size as inp
            input_size = inp.get_shape().with_rank(2)[1]   #with_rank表示确认是2维？
