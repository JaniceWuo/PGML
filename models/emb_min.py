...
class DiverEmbMin(BaseModel):
    def __init__(self, vocab, hps):
        super(DiverEmbMin, self).__init__(vocab, hps)   #init
    ...
    def add_batch_placeholder(self):
        self.enc_batch2 = tf.placeholder(tf.int32, [self.hps.batch_size * self.hps.meta_batch_size, None], name='enc_batch2')
        self.enc_lens2 = tf.placeholder(tf.int32, [self.hps.batch_size * self.hps.meta_batch_size], name='enc_lens2')
        self.enc_pad_masks2 = tf.placeholder(tf.float32, [self.hps.batch_size * self.hps.meta_batch_size, None], name='enc_pad_masks2') 
        self.dec_lens2 = tf.placeholder(tf.int32, [self.hps.batch_size * self.hps.meta_batch_size], name='dec_lens2')
        self.dec_inp_batch2 = tf.placeholder(tf.int32, [self.hps.batch_size * self.hps.meta_batch_size, self.hps.max_dec_len], name='dec_inp_batch2')
        self.dec_tgt_batch2 = tf.placeholder(tf.int32, [self.hps.batch_size * self.hps.meta_batch_size, self.hps.max_dec_len], name='dec_tgt_batch2')
        self.dec_pad_masks2 = tf.placeholder(tf.float32, [self.hps.batch_size * self.hps.meta_batch_size, self.hps.max_dec_len], name='dec_pad_masks2')

    def add_placeholder2(self):
        self.enc_batch = tf.placeholder(tf.int32, [self.hps.batch_size * self.hps.meta_batch_size, None], name='enc_batch')
        self.enc_lens = tf.placeholder(tf.int32, [self.hps.batch_size * self.hps.meta_batch_size], name='enc_lens')
        self.enc_pad_masks = tf.placeholder(tf.float32, [self.hps.batch_size * self.hps.meta_batch_size, None], name='enc_pad_masks') 
        self.dec_lens = tf.placeholder(tf.int32, [self.hps.batch_size * self.hps.meta_batch_size], name='dec_lens')
        self.dec_inp_batch = tf.placeholder(tf.int32, [self.hps.batch_size * self.hps.meta_batch_size, self.hps.max_dec_len], name='dec_inp_batch')
        self.dec_tgt_batch = tf.placeholder(tf.int32, [self.hps.batch_size * self.hps.meta_batch_size, self.hps.max_dec_len], name='dec_tgt_batch')
        self.dec_pad_masks = tf.placeholder(tf.float32, [self.hps.batch_size * self.hps.meta_batch_size, self.hps.max_dec_len], name='dec_pad_masks')
    ...
    def add_mechanism(self):   
        ...
        maxout_res = tf.contrib.layers.maxout(enc_feature, self.hps.hidden_dim)  
    ...
    def regularize_by_attn_dists_timestep(self):
        ...
        attnsum = tf.reduce_sum(attn, axis=1, keepdims=True)
    ...
    def dec_init_with_mechanism(self):
        ...
        # [batch_size * matrix_num, hidden_dim] with [state1, state1, ..... state 2, state2 ... state n, state n]
        #[160,256]
    ...
    def add_loss_calculation(self):
        ...
        self.word_loss = tf.negative(tf.reduce_mean(self.word_output))  #加入word的Loss
        
        
        self.loss = self.seq_loss + self.hps.emb_min_coeff * self.P + self.hps.word_min_coeff * self.word_loss

        # tf.summary.scalar('word_loss', self.word_loss)
    ...
    def add_decoder(self):   #decoder为单层GRU
        ...
        if self.hps.mode == 'decode':
            ...
            # print("emb_dec_inp's shape:",(emb_dec_inp))  #shape=(160, 300)
    ...
    def try_run(self,batch,sess,grad_op):
        feeddict = self.make_feeddict(batch)
        sess.run([grad_op], feeddict)
        # return loss
    ...
    def run_step(self, batch, sess, is_train=False, freeze_layer=False,val=False):
        feeddict = self.make_feeddict(batch)
        to_return = {
        ...
        if val:
            val_batch_loss = self.loss
            return sess.run(to_return, feeddict),val_batch_loss
        else:
